...
//---Función que quita un vuelo de la lista que contiene las id de los vuelos restantes---//
...
Struct siguiente_vuelo(int ultimo_vuelo, int num_vuelos, double tiempo_espera_min, double tiempo_espera_max,
    ...
    int vuelos_disponibles_tempc_no_a_base [tamano_listas] = {0}; //tempc que no van a base, se reacomodarán random.
    ...
    //El algoritmo manejará las restricciones de tiempo máximo de vuelo y tiempo máximo de trabajo
        ...
        if (horas_restantes>=3 && horas_vuelo_restantes>=2){ //Si le quedan más de 3 horas restantes...
            int random;
    ...
    if (ret.next_flight>0 && ret.atascado == false && ret.fin_tripulacion == false && size_vuelos_disponibles_tempc>1){ //Si eligió un vuelo válido..
        ...
        for (int i = 0; (i<size_vuelos_disponibles_tempc); i++){
            ...
            if (strcmp(destino, base_tripulacion) == 0 ){ //Encontró que sí hay vuelo, entonces hay retorno = true.
                ret.hay_retorno = true;
        ...
        //--Backup de la lista de vuelos disponibles c, ordenados random y dejando al último los que lo lleven a base
        if (ret.hay_retorno == true && size_vuelos_disponibles_tempc>1){
            //Reacomodar lista de vuelos no a base de forma random y luego agregarle al final los que sí lo llevan a base.
